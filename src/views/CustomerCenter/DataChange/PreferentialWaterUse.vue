...
<div v-if="PreferentialWaterUseVisible" class="PreferentialWaterUse">
  <!-- 弹出表单选择用户 -->
    ...
    <div class="bread-contain kr-top">
    ...
    <PreferentialWaterUseEdit :isEdit="isEdit" ref="childPreferentialWaterUse" class='kr-buttom'></PreferentialWaterUseEdit>
</div>

  <div class="PreferentialWaterUse" v-else>
    <!-- 弹出表单编辑 -->
      ...
      <el-form :inline="true" size="mini" :model="formData" class="demo-form-inline">
        ...
        <el-form-item class="form-left">
          ...
          <el-form-item label="优惠策略：">
            <el-select clearable v-model="formData.cheapList" placeholder="请选择">
              <el-option v-for="(item,index) in formData.cheapList" :key="index" :label="item.name" :value="item.value"></el-option>

            </el-select>
          </el-form-item>
          <el-form-item label="模糊查询：" class="searchInput">
            <!-- <el-input v-model="formData.con" clearable placeholder="用户编号/用户名称/分类项"></el-input> -->
            <el-tooltip class="item" effect="dark" content="用户编号、用户名称、分类项" placement="top">
    ...
    <div class="kl-table">
      <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="histroyData.list" class="change-tables-table">
        ...
        <el-table-column prop="areaName" min-width="150" label="审批状态">
        </el-table-column>

        <el-table-column prop="connect1" min-width="100" label="受理日期">
        </el-table-column>

        <el-table-column prop="connect2" min-width="100" label="受理单号">
        </el-table-column>

        <el-table-column prop="daytime" min-width="180" label="用户编号">

        </el-table-column>

        <el-table-column prop="factoryName" min-width="200" label="用户名称">
        </el-table-column>

        <el-table-column prop="isxz" min-width="180" label="用户地址" show-overflow-tooltip>
        </el-table-column>

        <el-table-column prop="meter" min-width="80" label="低保编号">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="80" label="优惠策略">
        </el-table-column>

        <el-table-column prop="flux" min-width="80" label="开始日期">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="80" label="结束日期">
        </el-table-column>

        <el-table-column prop="model" min-width="80" label="优惠备注">
        </el-table-column>
...
<script>
import PreferentialWaterUseEdit from './PreferentialWaterUseEdit'
import chooseuser from "../Transfer/chooseuser"
export default {
  name: "PreferentialWaterUse",
  components: {
  ...
  data() {
    return {
      ...
      crumbsData: {  //面包屑
        titleList: [
          { title: '客户中心', path: '/PreferentialWaterUse' },
          { title: '变更', method: () => { window.histroy.back() } },
          { title: '优惠用水变更', method: () => { window.histroy.back() } }
        ],
      ...
      PreferentialWaterUseVisible: false,  //综合查询的显示和隐藏
      PreferentialWaterUseName: '优惠用户资料-增加',
      ruleFormData: {},
  ...
  methods: {
    ...
    this.crumbsData.titleList.pop();
      this.PreferentialWaterUseVisible = false;
    ...
    //弹出框
    submit(formName) {	//提交
      this.$refs.childPreferentialWaterUse.submit()
      // this.PreferentialWaterUseVisible = false;
    },
    add(val) {
      if(val==="add"){
      ...
      this.PreferentialWaterUseVisible = true;
    },
    // 删除行
    deleteRow(index, row) {
