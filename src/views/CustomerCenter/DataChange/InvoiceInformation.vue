...
<template>
  ...
  <div class="InvoiceInformation" v-if="PaymentMethodVisible">
    <!-- 弹出表单选择用户 -->
    ...
    <InvoiceInformationEdit ref="childPaymentMethod" v-if="editShow"></InvoiceInformationEdit>
    <InvoiceInformationAdd v-else></InvoiceInformationAdd>
  </div>

  <div class="InvoiceInformation" v-else>
    <div class="bread-contain">
      <publicCrumbs :crumbsData="crumbsData"></publicCrumbs>
      <!-- <h2 class="tab-title">
            <i></i>
    ...
    <div class="kl-table">
      <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="histroyData.list" class="change-tables-table">
        ...
        <el-table-column prop="fixValue" min-width="100" label="发票类型">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="100" label="单位名称">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="100" label="纳税人识别号">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="100" label="联系地址">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="100" label="座机号码">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="100" label="开户行">
        </el-table-column>

        <el-table-column prop="fixValue" min-width="100" label="银行账号">
        </el-table-column>
...
<script>
import InvoiceInformationEdit from './InvoiceInformationEdit'
import chooseuser from "../Transfer/chooseuser"
import InvoiceInformationAdd from "./InvoiceInformationAdd"
export default {
  name: "InvoiceInformation",
  components: {
    InvoiceInformationEdit,
    chooseuser,
    InvoiceInformationAdd
  },
  data() {
    return {
      editShow:false,
      ...
      crumbsData: {  //面包屑
        titleList: [
          { title: '客户中心', path: '/InvoiceInformation' },
          { title: '变更', method: () => { window.histroy.back() } },
          { title: '开票资料变更', method: () => { window.histroy.back() } }
        ],
