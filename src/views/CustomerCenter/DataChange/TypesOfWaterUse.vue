...
<template>
  <div class="TypesOfWaterUse" v-if="TypesOfWaterUseVisible">
    <!-- 弹出表单选择用户 -->
    ...
    <TypesOfWaterUseEdit ref="childTypesOfWaterUse" :isEdit="isEdit"></TypesOfWaterUseEdit>
    <!-- <div slot="footer" class="dialog-footer" ref="ruleForm">
  ...
  <div class="TypesOfWaterUse" v-else>
    <!-- 弹出表单编辑 -->
    ...
    <div class="kl-table">
      <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="histroyData.list" class="change-tables-table">
        ...
        <el-table-column prop="daytime" min-width="100" label="受理单号">
        </el-table-column>
        ...
        <el-table-column prop="lvalue" min-width="180" label="用户名称">
        </el-table-column>

        <el-table-column prop="lvalue" min-width="100" label="原用水类型">
        </el-table-column>
...
<script>
import TypesOfWaterUseEdit from './TypesOfWaterUseEdit'
import chooseuser from "../Transfer/chooseuser"
export default {
  name: "TypesOfWaterUse",
  components: {
    TypesOfWaterUseEdit,
    chooseuser
  },
  data() {
    return {
      ...
      crumbsData: {  //面包屑
        titleList: [
          { title: '客户中心', path: '/TypesOfWaterUse' },
          { title: '变更', method: () => { window.histroy.back() } },
          { title: '用水类型变更', method: () => { window.histroy.back() } }
        ],
  ...
  methods: {
    ...
    //弹出框
    submit(formName) {	//提交
      this.$refs.childTypesOfWaterUse.submit()
      // this.TypesOfWaterUseVisible = false;
    },
    edit(val) {
      if(val==="add"){
